...
| 攻击 | J 或 鼠标左键 | B |
| 剑气 | K | Y |
| 下移 | S 或 ↓ | 左摇杆下（空中+攻击=下劈） |
| 暂停 | Esc | Start |

键位可在「设置 → 操作」中重映射（只替换键盘，手柄绑定保留；可恢复默认）。

近战命中积攒魂量、按剑气消耗魂量倾泻新月弹（HK 式魂量循环，重生清零）；
...
有存档时主菜单会出现「继续游戏」（最近重生点满血落位）；
`bash Tools/export.sh` 一键导出 Windows 单文件版到 `bin/2d-platformer.exe`。
