...
>
> **当前进度（2026-09-06）**：**阶段⓪-⑥ 全部完成并勾记**（25 个单元测试 + 探针 8/8 + CI 绿），
> 工程地基六阶段收官。**下一步**：§5 下一期候选（1. UI 框架与开始游戏闭环；
> 2. 关卡加载与场景切换闭环）。实施时先写该期的详细计划（参照 `Docs/plans/` 既有格式），
> 审阅后再动手。
...
### 阶段④ 32×32 美术适配（世界 ×2）✅（2026-09-06 完成）
...
### 阶段⑤ Phantom Camera 接入与震屏打磨 ✅（2026-09-06 完成）
...
1. **UI 框架与开始游戏闭环**：主菜单/暂停/HUD 血条 + 菜单→游玩→死亡→重开的完整闭环
   ✅（2026-09-06 第二期交付，连同落坑即死与战斗代码审计加固，见 `Docs/plans/` 第二期计划）
2. **关卡加载与场景切换闭环**（相机边界随关卡走，衔接阶段⑤的相机基建）
   ✅（2026-09-06 第三期交付——经讨论升级为「单位穿透+接触伤害 + 九宫格房间流式无缝世界」，
   原 Level 队列/黑场过渡方案作废，见 `Docs/plans/` 第三期计划）

音频与正式美术资源随上述两项穿插。
（正式美术接入时同步把受击盒收贴最终剪影，玩家侧宁小勿大——当前占位受击盒 28×32 大于可见角色，
系 16×16 时代比例的延续；收紧后需重标定 `SlimeAIInputSource._attackDistance`，命中可达距离随之变化。
另：AI 在玩家无敌帧期间仍会发起攻击并空挥——随 AI 行为完善一并处理。
另：房间几何迁移 TileMapLayer 已定（随正式美术素材到位执行——绘制成本远低于手摆 StaticBody+ColorRect，
碰撞随 TileSet 自动生成），当前占位房间继续服役。）

## 5.1 第二期（2026-09-06 完成）

候选 1 之外新增两项，合并交付（计划见 `Docs/plans/2026-09-06-phase2-ui-loop-pit-death-combat-audit.md`）：

- **UI 闭环**：Main 应用编排 + 主菜单/暂停菜单（Esc 暂停树）+ HUD 心形血条（HealthChanged 驱动）
- **落坑即死**：`Gameplay/World/KillZone`（不扣血的环境即死，玩家重生、敌人消失），探针 8→9 项
- **战斗审计加固**：攻击数值接通配置单一事实源（原 Hitbox 导出死配置）、命中确认改为真实结算
  触发（拒伤不震屏）、`Health.HealthChanged` 事件、重生相机瞬移（PCam teleport_position）

## 5.2 第三期（2026-09-06 完成）

候选 2 升级交付（计划见 `Docs/plans/2026-09-06-phase3-level-loading.md`，经讨论定案）：

- **碰撞模型改造**：物理层五层规范化（terrain/enemy_body/player_hurtbox/enemy_hurtbox/
  player_body，修复层值与层名错位）+ 单位间全穿透（消除互挤/站头）+ 敌人接触伤害
  （ContactDamager 常驻，无敌帧限频，数值复用 AttackConfig）
- **九宫格房间流式无缝世界**：1280×720 房间网格、3×3 加载窗、RemoveChild 缓存保状态、
  世界级相机边界（相机可水平卷轴）、重生点随房间、门=墙体几何开口；验证世界三间房；
  探针切 World 场景
